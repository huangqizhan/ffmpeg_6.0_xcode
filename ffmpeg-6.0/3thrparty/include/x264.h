...
typedef struct x264_param_t
{
    /* CPU flags */
    uint32_t    cpu;
    /* 并行线程的数量，即同时被编码的图像的数量，增加线程数量会影响编码速度，
        * 同时也会影响编码延迟（所谓编码延迟就是 编码器中得到第一个输出时已经往输入的帧数）
        * 以帧为单位进行多线程编码
        */
    int         i_threads;           /* encode multiple frames in parallel */
    /* 向前看线程的数量，为了提高压缩率得到更好的质量，x264会缓存一部分帧用于分析
        * 以得到最优的编码参数，但是会增加编码延迟
    */
    int         i_lookahead_threads; /* multiple threads for lookahead analysis */
    /* 是否使用基于slice的线程，使用zerolatency将会禁用该选项，和i_threads是互斥的，即frame级别的并性
     * 和slice级别的并行不能同时进行
     * 以slice为单位进行多线程编码
    */
    int         b_sliced_threads;  /* Whether to use slice-based threading. */
    /* 当进行并行处理时，是否使用使用非确定的优化，默认是true
         */
    int         b_deterministic; /* whether to allow non-deterministic optimizations when threaded */
    /* 是否使用cpu相关的优化算法，如果否，那么选择传统的规范的算法（性能可能不高）
        */
    int         b_cpu_independent; /* force canonical behavior rather than cpu-dependent optimal algorithms */
    
    /* 前向缓存的帧的数量，前向缓存的目的是为了得到更好的比特率或画面质量
         */
    ...
    // yuv 分量的位深
    int         i_bitdepth;
    /*取值范围10-51。 设置比特流的Level。默认40，即4.0。 用来告诉解码器需要支持的什么级别的兼容性
     */
    int         i_level_idc;
     ...
     * 检验编码器产生的符合该标准的NAL单元流或字节流的偏差值。蓝光视频、电视广播及其它特殊领域有此要求
     */
    int         i_nal_hrd;
    // 视频信息
    struct
    {
        ...
        int         i_vidformat;
        /*
        Color Range 对于一个 8bit 的 RGB 图像，它的每一个 R、G、B 分量的取值按理说就是 0~255 的。但是真的是这样的吗？其实不是的。这里就涉及到 Color Range 这个概念。Color Range 分为两种，一种是 Full Range，一种是 Limited Range。Full Range 的 R、G、B 取值范围都是 0～255。而 Limited Range 的 R、G、B 取值范围是 16～235。
    ...
    // 设定x264输出的IDR帧(即关键帧)之间的最大间隔
    int         i_keyint_max;       /* Force an IDR keyframe at this interval */
    // 设定x264输出的IDR帧(即关键帧)之间的最小间隔
    int         i_keyint_min;       /* Scenecuts closer together than this are coded as I, not IDR. */
    // 设定I/IDR帧放置的阀值 用于衡量每一帧与前一帧的差异程度。若该值小于scenecut，则检测到'场景切换'('scenecut')条件，并放置一个I帧
    ...
    */
    ...
    // nalu是否符合标准 值为true，则NALU之前是4字节前缀码0x00000001;值为false，则NALU之前的4个字节为NALU长度
