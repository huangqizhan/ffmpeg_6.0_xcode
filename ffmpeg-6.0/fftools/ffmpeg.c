...
void term_init(void)
{
...
#if HAVE_TERMIOS_H
    /*
     tty.c_iflag &= ~(IGNBRK|BRKINT|PARMRK|ISTRIP|INLCR|IGNCR|ICRNL|IXON);：禁用输入模式标志位，包括忽略 BREAK 信号、BREAK 中断、标记错误、去除第 8 位、将 NL 转换为 CR、忽略 CR、将 CR 转换为 NL、禁用 XON/XOFF 流控制。
     tty.c_oflag |= OPOST;：启用输出模式标志位，包括处理输出数据，如回车换行等。
     tty.c_lflag &= ~(ECHO|ECHONL|ICANON|IEXTEN);：禁用本地模式标志位，包括回显字符、回显 NL、启用规范模式、启用扩展模式。
     tty.c_cflag &= ~(CSIZE|PARENB);：禁用控制模式标志位，包括字符大小掩码、启用奇偶校验。
     tty.c_cflag |= CS8;：设置字符大小为 8 位。
     tty.c_cc[VMIN] = 1;：设置非规范模式下的读取最小字符数为 1。
     tty.c_cc[VTIME] = 0;：设置非规范模式下的读取超时时间为 0。
     */
    if (stdin_interaction) {
        ...
        if (tcgetattr (0, &tty) == 0) {
            ...
            tty.c_cc[VMIN] = 1;
            tty.c_cc[VTIME] = 0;
//这段代码的作用是获取当前终端的属性，然后修改终端的属性，禁用一些特定的终端模式，并设置一些终端参数。
...
static BenchmarkTimeStamps get_benchmark_time_stamps(void)
{
    ...
    getrusage(RUSAGE_SELF, &rusage);
     //struct timeval ru_utime; /* 用户空间时间 */
     //struct timeval ru_stime; /* 内核空间时间 */
...
static int64_t getmaxrss(void)
{
    ...
    struct rusage rusage;
    getrusage(RUSAGE_SELF, &rusage);
    //常驻内存集是指进程当前实际使用的物理内存大小。  乘以1024，是为了将其单位从KB转换为字节
